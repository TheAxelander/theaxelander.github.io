...
After that, the next iteration starts and for every month the expected amount will be calculated again. There is no need to update the target date in the config for this bucket (as said it's only for the first target date).

#### Save X until Y date

This bucket type you can use if you want to save a fixed amount of money every month until a specific target date.
...
To the define the opening balance, go to `Transaction Page` and create a new Bank Transaction. Set the date on which you want to start tracking your transactions and enter the balance from that day as amount. Assign this record to the Bucket `Income` like below.

![Screenshot 4](assets/screenshot4.png)

### Create buckets

With creating your buckets on the `Bucket Page`, you immediately start your financial planning. Think about what kind of expenses you have, how you could group them and how they occur.
...
After that, export some Transactions from your Online Banking and import the data using `Import Page`. At the moment it support CSV files only, but you can individually set the characters for delimiter and text qualifier. The respective settings and other options are shown once the CSV file has been uploaded and you have filled the form of each step.

![Screenshot 3](assets/screenshot3.png)

Following all steps in the wizard, click on `Validate Data` on the last screen and verify the output. If everything works well, you should see some figures like below. If you are happy, click on `Import Data` to finish the process.

![Screenshot 11](assets/screenshot11.png)

### Bucket Assignment

In the final step you assign your imported Transactions to certain Buckets. Go back to the `Transaction Page`, switch to Edit mode and select an appropriate Bucket. If a Transaction should belong to more than one Bucket, just reduce the assigned amount and you get automatically the option to assign the remaining amount to another Bucket.

![Screenshot 5](assets/screenshot5.png)

Transactions which represent your (monthly) income can be assigned to the pre-defined `Income` Bucket. If you have transferred money from one Account to another, you can use the `Transfer` Bucket. Please ensure that all `Transfer` Transactions have in total a 0 Balance to prevent data inconsistency and wrong calculations.

![Screenshot 12](assets/screenshot12.png)
...
![Screenshot 13](assets/screenshot13.png)

### Bucket History

OpenBudgeteer has a built-in versioning for Buckets which enables a proper historical view on previous months. If you modify a Bucket, like changing the Type or the Target Amount, it will create a new version for the current selected month. It is not recommended to change a Bucket in the past, a change between two Bucket Version is prevented.

If you close a Bucket it will be marked as `Inactive` for the upcoming month. This can be only done if the Bucket Balance is 0 to prevent wrong calculations.
